...
class TestABCDParams(unittest.TestCase):
    ...
    def test_sort(self):
        p1 = ABCDParams(w=np.array([1, 2, 3]), s=np.array([0, 2, 1]), mu=0.5, has_outliers=False)

        assert p1.w.tolist() == [3, 2, 1]
        assert p1.s.tolist() == [2, 1, 0]

        p2 = ABCDParams(w=np.array([1, 2, 3, 0, 7]), s=np.array([0, 2, 1, 2]), mu=0.5, has_outliers=True)
